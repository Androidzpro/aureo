...
export default function CalendarPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 pb-8">
      ...
      <div className="bg-white dark:bg-[#1A1A1A] rounded-2xl border border-gray-100 dark:border-gray-800 p-3">
        <Calendar value={selected} onChange={(v) => setSelected(v as Date)} locale="es-MX" />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-gray-100 dark:border-gray-800 p-3"><p className="text-[10px] text-gray-400 uppercase">Ingresos</p><p className="text-sm font-bold text-emerald-600">{formatCurrency(dayIncome)}</p></div>
        <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-gray-100 dark:border-gray-800 p-3"><p className="text-[10px] text-gray-400 uppercase">Gastos</p><p className="text-sm font-bold text-red-600">{formatCurrency(dayExpense)}</p></div>
      </div>
      {loading ? <div className="text-center py-8 text-gray-300 text-xs">Cargando...</div>
        : dayTxs.length === 0 ? <div className="bg-white dark:bg-[#1A1A1A] rounded-2xl border border-gray-100 dark:border-gray-800 p-8 text-center"><p className="text-xs text-gray-400">Sin movimientos este día</p></div>
