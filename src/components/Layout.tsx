    ...
    <div className="min-h-screen bg-[#FAFAFA]">
          ...
          <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'tween', duration: 0.2 }}
            ...
            <nav className="p-2 space-y-0.5">
              {navItems.map(item => (
                <Link key={item.path} to={item.path} onClick={() => setMobileOpen(false)}
                  className={cn('flex items-center gap-2.5 px-3 py-1.5 rounded-md text-sm transition-colors',
                    location.pathname === item.path ? 'bg-[#F0F0F0] text-[#1A1A1A] font-medium' : 'text-[#5C5C5C] hover:bg-[#F5F5F5] hover:text-[#1A1A1A]')}>
              ...
              <Link to="/settings" onClick={() => setMobileOpen(false)}
                className={cn('flex items-center gap-2.5 px-3 py-1.5 rounded-md text-sm transition-colors',
                  location.pathname === '/settings' ? 'bg-[#F0F0F0] text-[#1A1A1A] font-medium' : 'text-[#5C5C5C] hover:bg-[#F5F5F5] hover:text-[#1A1A1A]')}>
      ...
      <aside className="hidden lg:flex fixed left-0 top-0 h-screen w-[240px] bg-white border-r border-[#EAEAEA] flex-col z-50">
        ...
        <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto">
          <p className="px-3 pt-3 pb-1.5 text-[11px] font-medium text-[#A0A0A0] uppercase tracking-[0.04em]">General</p>
          {navItems.map(item => (
          ...
          <div className="border-t border-[#EAEAEA] my-2" />
          <p className="px-3 pt-2 pb-1.5 text-[11px] font-medium text-[#A0A0A0] uppercase tracking-[0.04em]">Cuenta</p>
          <Link to="/settings" className={cn(location.pathname === '/settings' ? 'sidebar-link-active' : 'sidebar-link')}>
        ...
        <div className="p-2 border-t border-[#EAEAEA]">
          <div className="flex items-center gap-2.5 px-3 py-2">
            <div className="w-6 h-6 rounded-full bg-[#F0F0F0] border border-[#EAEAEA] flex items-center justify-center">
              <span className="text-[11px] font-semibold text-[#5C5C5C]">{profile?.full_name?.charAt(0)?.toUpperCase() || 'U'}</span>
            </div>
          ...
          <button onClick={logout}
            className="w-full flex items-center gap-2.5 px-3 py-1.5 rounded-md text-sm text-[#5C5C5C] hover:bg-red-50 hover:text-red-600 transition-colors">
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-[#EAEAEA] z-40">
        <div className="flex justify-around py-2">
          {[navItems[0], navItems[1], navItems[3], navItems[5]].map(item => {
            ...
            return (
              <Link key={item.path} to={item.path}
                className={cn('flex flex-col items-center py-1 px-3 min-w-[56px]',
                  active ? 'text-[#1A1A1A]' : 'text-[#A0A0A0]')}>
                <item.icon size={18} />
