...
import { Home, Receipt, CalendarDays, TrendingUp, CreditCard, Target, Settings, LogOut, X } from 'lucide-react'
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-2xl border-b border-gray-100/50">
        <div className="flex items-center justify-between px-5 py-3">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-lg font-extrabold bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 bg-clip-text text-transparent">FlowFin</span>
          </div>
              ...
              <div className="p-6 pb-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-xl overflow-hidden shadow-lg shadow-indigo-200">
                      <img src="/logo.svg" alt="FlowFin" className="w-full h-full object-cover" />
                    </div>
                    <div>
                      <span className="text-xl font-extrabold bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 bg-clip-text text-transparent">FlowFin</span>
                  ...
                  return (
                    <Link key={item.path} to={item.path} onClick={() => { setMobileOpen(false); playSound('click') }}
                      className={cn('flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-sm font-medium',
                        active ? 'bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white shadow-lg shadow-indigo-200' : 'text-gray-600 hover:bg-gray-50')}>
